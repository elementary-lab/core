...
export abstract class Cache implements CacheInterface {
    ...
    public async get<T>(key: string): Promise<T | boolean> {
    ...
    public add(key: string, value: any, duration: number): Promise<any> {
    ...
    public getOrSet(key: string, callable: () => void, duration?: number): boolean {
        // TODO Implement logic
        return false;
    }

    public multiAdd(items: any[], duration: number): number {
        // TODO Implement logic
        return 0;
    }

    public multiGet(keys: string[]): any[] {
        return [];
    }

    public multiSet(items: any[], duration: number): number {
        // TODO Implement logic
        return 0;
    }

    public set(key: string, value: any, duration?: number): Promise<boolean> {
        key = this.buildKey(key);
     ...
     * @param value mixed: any the value to be cached. Most often it's a string. If you have disabled [[serializer]],
     * it could be something else.
     * @param duration number the number of seconds in which the cached value will expire. 0 means never expire.
     * @return bool true if the value is successfully stored into cache, false otherwise
     */
     ...
     * @param key string the key of the value to be deleted
     ...
     * @param key string[] a list of keys identifying the cached values
     ...
     * @return array array of failed keys
     */
    protected addValues(data, duration: number): void {
        // failedKeys = [];
        // foreach (data as key => value: any) {
